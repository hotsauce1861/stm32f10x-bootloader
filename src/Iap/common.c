...
{
	int32_t abort = 0;
	uint32_t	boot_delay = BOOT_DELAY*SYSTICK_FREQ;
	uint8_t key = 0;
	uint32_t  time = 0;
	...
	gw_hal_set_time_cnt((uint32_t)boot_delay);
	...
	if(boot_delay > 0){
		...
		Int2Str(str_buf, boot_delay/SYSTICK_FREQ);
		SerialPutString(Int2Str);		
	}
  
	while(!abort){		
		do{
			boot_delay = gw_hal_get_time_cnt();
		...
		}while((!abort) && (boot_delay - last_time < SYSTICK_FREQ-1));
		last_time = boot_delay;
	...
	while (1)
	{
		...
		else if (key == CMD_UPLOAD)
		{
			/* Upload user application from the Flash */
			SerialUpload();
		}
		else if (key == CMD_LOAD_APP)
		{
			SerialPutString("Start Application \r\n\n");
		...
		else if ((key == CMD_DISABLE_FLASH_PROTECTION) && (FlashProtection == 1))
		{
